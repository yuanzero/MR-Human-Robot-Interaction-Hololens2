...
/*
 * This code is created by LIAO ZHONGYUAN from HKUST
 * Modified from class ObjectManipulator.cs
 * function description: selected the object in near or far, but do not perform transform.
 * 2023.3.17 finished the first sucesseed version.
 * remark:
 * 1. in the interface setting, the is no difference to choose the one hand or tow hand type, but do not choose nothing. 
 * if we select nothing in the manipulation type, we can not enter the manippulation even. in the previous stategy, we use hover event.
 * 2. used the manupulationStart even to start the manipulation, it will perfoem the transform of object in class ObjectManipulator.cs
 * 3. the manipulation is caculated in ROS system, so cancel all the transformation
 * 4. the code need the rigidbody in the same object.
 * 5. need collider, in subobject or the same, in the robot case is mesh collider. in the same object, NearInteractionGrabbable is needed
 * 6. public static bool isPointerDown = false; //define a bool to defined pointer's up/down. USE this static type to convert variable acrossing scripts.
 */
...
namespace Microsoft.MixedReality.Toolkit.UI
{
    ...
    public class ObjectManipulatedDetect : MonoBehaviour, IMixedRealityPointerHandler, IMixedRealityFocusChangedHandler
    {
        #region Public Enums
        ...
        public static bool isPointerDown = false; //define a bool to defined pointer's up/down.
        //public static bool isOneHand = false; //define a bool to defined hand.
        ...
        */
        ...
        private void Start()
        {
            rigidBody = HostTransform.GetComponent<Rigidbody>();  // i have cancel all the digitialbody related function, since it no need to transform
        ...
        public void OnPointerDown(MixedRealityPointerEventData eventData)
        {
            ...
            //Debug.Log("second_check allowFarManipulation = true");
            ...
            if (manipulationType != ManipulationHandFlags.OneHanded || pointerIdToPointerMap.Count == 0)
            {
                ...
                if (!pointerIdToPointerMap.ContainsKey(id))
                {
                    ...
                    else if (IsOneHandedManipulationEnabled)
                    {
                        ...
                        Debug.Log("HandleOneHandMoveStarted000000000000000000000000000000000");
                        HandleOneHandMoveStarted();
                    }
                    //isOneHand = true; // debug
                    */
        ...
        */
        ...
        private void HandleManipulationStarted()
        {
            ...
            Debug.Log("isManipulationStarted = true0000000000000000000000");
        ...
        private void HandleManipulationEnded(Vector3 pointerGrabPoint, Vector3 pointerVelocity, Vector3 pointerAnglularVelocity)
        {
            ...
            Debug.Log("isManipulationStarted = false00000000000000000000");
            // TODO: If we are on HoloLens 1, push and pop modal input handler so that we can use old
            // gaze/gesture/voice manipulation. For HoloLens 2, we don't want to do this.
