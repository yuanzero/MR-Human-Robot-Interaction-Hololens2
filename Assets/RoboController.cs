...
public class UR5_Solver
{
    ...
    private float DH_ratio = 0.1f / 0.425f; //taken from length of DH_a
...
public class RoboController : MonoBehaviour
{
    ...
    public float x = 0.06f, y = 0.4f, z = 0.13f, phi = 0, theta = 0, psi = 0, oldX, oldY, oldZ, oldPhi, oldTheta, oldPsi;
    ...
    void LateUpdate()
    {
        Vector3 Target = controlCube.transform.position - RobotBase.transform.position;
        x = Target.x;
        y = Target.y;
        z = Target.z;
    ...
    */
    ...
    void initializeJoints()
    {
        ...
        {
            if (RobotChildren[i].name == "joint2")
            {
