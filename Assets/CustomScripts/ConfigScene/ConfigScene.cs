...
public class ConfigScene : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            PlayerPrefs.SetString("server_address", "ws://192.168.1.104:9090");
        }
    ...
    private async void AcceptInput(string ip, string azureId, string azureKey, string azureDomain)
    {
        ...
        //await sceneSystem.LoadContent("CR5demosene", LoadSceneMode.Single);
        await sceneSystem.LoadContent("gripper", LoadSceneMode.Single); //change the sence when modify another sence
    }
