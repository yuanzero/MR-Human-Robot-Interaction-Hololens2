...
public class Robot_kenimatic : MonoBehaviour
{
    ...
    void Update()
    {
        try
        {
            ...
            if (Target.transform.localPosition != lastPos || Target.transform.localRotation != lastRot)
            {
                ...
                int i = 0;
                foreach (var value in currentAngles)
                {
    ...
    private void SetQ(double[] q)
    {
        ...
        for (int i = 0; i < transformations.Count - 1; i++)
        {
            ...
            try
            {
                Joints[i].transform.localRotation = FromRotationMatrx(R).rotation; //perform rotation
            }
            catch
            {
